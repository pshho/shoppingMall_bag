...
public class MemberController {
	...
	int idCheck(HttpServletRequest request) {
		...
		int quitMemChk = memMapper.quitMemberCheck(userId);

		return idCheck + quitMemChk;
	}
	...
	public MemberController() {
		...
		this.messageService = NurigoApp.INSTANCE.initialize("NCSVLIX3AOOHHTOQ", "ZFYWIZR78FZ1LL4EMF0BHF1EACVQUKPG",
	...
	String findPw(HttpServletRequest request) {
		MemberDTO mdto = memMapper.findPw(request.getParameter("id"), request.getParameter("name"),
	...
	String myPage(HttpSession session, Model mm) {
		...
		mm.addAttribute("memberService", "myPage");
		return "member/template";
	}

	// 마이페이지
	@PostMapping("myPageReg")
	String myPageReg(HttpSession session, MemberDTO mdto, AddressDTO adto, Model mm) {
		// System.out.println(session.getAttribute("userId"));
	...
	int checkAddr(HttpSession session) {
		String userId = (String) session.getAttribute("userId");

		List<AddressDTO> getUserAddr = addrMapper.getUserAddress(userId);

		return getUserAddr.size();
	}
	...
	boolean deleteAddress(HttpServletRequest request, HttpSession session) {
		String memberId = (String) session.getAttribute("userId");
		String addressId = request.getParameter("addressId");
		List<AddressDTO> getUserAddr = addrMapper.getUserAddress(memberId);
		if (getUserAddr.size() > 1) {
